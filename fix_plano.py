...
# Add State
state_decl = "  const [showBatchDeleteConfirm, setShowBatchDeleteConfirm] = useState(false);\n"
if "showBatchDeleteConfirm" not in content:
...
old_batch = """  const handleBatchDelete = async () => {
...
new_batch = """  const handleBatchDelete = async () => {
    if (selectedActivityIds.length === 0) return;
