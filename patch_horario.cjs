...
const targetFunction = `  const gerarHorario = () => {
    setLoading(true);
    setLogs([]);
    setHorarios([]);
...
const replacementFunction = `  const gerarHorario = () => {
    setLoading(true);
    setLogs([]);
    setHorarios([]);
...
if (content.includes(targetFunction)) {
  ...
  console.log("Replaced gerarHorario function");
} else {
  console.log("Could not find targetFunction");
}
