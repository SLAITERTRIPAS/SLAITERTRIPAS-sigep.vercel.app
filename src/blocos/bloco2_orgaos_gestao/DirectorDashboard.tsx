...
export default function DirectorDashboard({
  ...
  return (
    <div className="flex h-full bg-gray-50 flex-col md:flex-row overflow-hidden font-sans relative">
      ...
      >
        <div className={`flex-1 flex flex-row md:flex-col space-y-0 md:space-y-2 gap-2 md:gap-0 min-w-max md:min-w-0 ${!isSidebarOpen && 'md:hidden'}`}>
          {menuItems.map((item) => (
            <button
              key={item.title}
      ...
      <div className={`flex-1 overflow-y-auto flex flex-col relative w-full h-full ${
        ...
        {(activeItem === "Plano de Actividades" ||
          ...
          selectedPlanType !== "Plano Individual" && (
            <button
              onClick={handleBack}
              className={`mb-6 text-blue-600 font-black text-xs tracking-widest hover:underline flex items-center gap-2 self-start ${!isSidebarOpen ? 'md:ml-12' : ''}`}
            >
        ...
        {activeItem !== "Caixa de Mensagens" && activeItem !== "Estatística" && activeItem !== "Bolsa de Estudos" && (
          <h2 className={`text-2xl font-bold text-slate-800 mb-6 font-serif tracking-tight transition-all ${!isSidebarOpen ? 'md:ml-12' : ''}`}>
            {title} - {activeItem}
          </h2>
        )}
